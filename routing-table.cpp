...
{
  const RoutingTableEntry* longestPrefixMatch = nullptr;
  for (const auto& entry : m_entries)
  {
    if ((entry.mask & ip) == (entry.mask & entry.dest))
    {
      if (longestPrefixMatch == nullptr || entry.mask >= longestPrefixMatch->mask)
        longestPrefixMatch = &entry;
    }
  }
  if (longestPrefixMatch == nullptr)
